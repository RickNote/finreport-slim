...
所有脚本均在本 skill 的 `scripts/` 目录下，`<skill_dir>` 指本 skill 的根目录。
...
- API Key 自动从 `<skill_dir>/config.env` 加载，无需手动设置
- 输出目录：`<output_root>/<pdf_stem>/`，包含 `<stem>.pages.json`、`<stem>.md`、`<stem>.candidates.json`

高级选项参见 [references/commands.md](references/commands.md) 的 `convert` 参数部分。

---

## 第二步：选择主题

读取 `<skill_dir>/scripts/themes.py` 中的 `THEME_PRESETS` 字典，列出所有可用主题名称和 description，让用户选择。

**如果没有合适的预设主题**：让用户提供几个关键词，提醒用户若效果理想可将关键词添加到 `scripts/themes.py` 的 `THEME_PRESETS` 字典中永久保存。格式参考 `scripts/themes.example.py`。
...
> **注意**：运行 `finreport_scope.py` 时，Python 需要能找到同目录下的 `themes.py`。直接用绝对路径调用脚本即可（无需 cd）。
